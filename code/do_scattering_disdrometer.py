...
jw_disdrometer_file_path = '/lcrc/group/earthscience/rjackson/disdrometer/twpdisdrometer*.cdf'

def brandes2002(D_eq):
    return (0.9951 + 0.0251*D_eq - 0.03644*D_eq**2 + 0.005303*D_eq**3 -
            0.0002492*D_eq**4)

def get_PSD(filename):
    try:
        JWReader = pydsd.read_arm_jwd_b1(filename)
...
if __name__ == '__main__':
    ...
    num_drop = np.ma.concatenate([x.fields['num_drop']['data'] for x in the_PSDs])
    ...
    dros_class = np.stack(dros_class)
    mjo_index = time_bag.map(get_mjo_index).compute()
    mjo_index = np.stack(mjo_index)           
    ...
    print(ds)
    ds.to_netcdf('PSDs_w_radar_params.cdf')
